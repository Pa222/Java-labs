...
<div class="container" th:align="center">
    <h1>List of games:</h1>
    <form th:action="@{/editpublishers}" th:object="${filters}" method="GET">
        <div>
    ...
    <table th:each="publisher : ${publishers}">
        <tr>
            <td class="table__col" th:utext="${publisher.id}"></td>
            <td class="table__col" th:utext="${publisher.publisherName}"></td>
            <td class="table__col">
                <form th:action="@{/deletepublisher}" th:object="${Publisher}" method="POST">
                    <input type="hidden" name="id" th:value="${publisher.id}">
                    <input type="submit" value="Удалить">
                </form>
            </td>
            <td class="table__col">
                <form th:action="@{/editpublisher}" th:object="${Publisher}" method="GET">
                    <input type="hidden" name="id" th:value="${publisher.id}">
